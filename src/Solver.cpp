...
std::optional<double>
sauron::Solver::triangleSolver(sauron::Ray &ray,
                               std::vector<sauron::Point> &edges) const {

  const auto edge1 = edges[1] - edges[0];
  const auto edge2 = edges[2] - edges[0];

  const auto ray_cross_eage_2 = sauron::cross_product(ray._direction, edge2);
  const double det = sauron::dot_product(edge1, ray_cross_eage_2);

  if (std::fabs(det) < EPSILON)
    return std::nullopt;

  const double invDet = 1.0 / det;
  auto T = ray._starting_point - edges[0];

  const double u = dot_product(T, ray_cross_eage_2) * invDet;
  if ((u < 0 && std::fabs(u) > EPSILON) ||
      (u > 1 && std::fabs(u) * (u - 1) > EPSILON))
    return std::nullopt;

  const auto T_cross_eage_1 = cross_product(T, edge1);
  const double v = sauron::dot_product(ray._direction, T_cross_eage_1) * invDet;

  if ((v < 0 && std::fabs(v) > EPSILON) ||
      (u + v > 1 && std::fabs(u + v - 1) > EPSILON))
    return std::nullopt;
...
std::optional<double>
sauron::Solver::quadSolver(sauron::Ray &ray,
                           std::vector<sauron::Point> &edges) const {
  ...
  auto solution2 = triangleSolver(ray, triangle2);
